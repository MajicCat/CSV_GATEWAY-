...
int hmacsign( const char *keyfile, const char *infile, const char *outfile, unsigned int blocksize, int signature_only )
{
...
void usage( void )
{
    printf( "Usage: hmac_sha256 <mode>\n"
        ...
        "  sign <keyfile> <infile> <signedfile> [blocksize]   Sign the supplied file\n"
...
int main( int argc, const char **argv )
{
    ...
    else if ( strcmp( mode, "sig" ) == 0 )
    {
        ...
        if ( hmacsign( argv[ argn ], argv[ argn + 1 ], argv[ argn + 2 ], 0, 1 ) == 0 )
        {
            return -1;
        }
    }
    else if ( strcmp( mode, "sign" ) == 0 )
    {
        ...
        if ( hmacsign( argv[ argn ], argv[ argn + 1 ], argv[ argn + 2 ], blocksize, 0 ) == 0 )
        {
            return -1;
        }
    }
