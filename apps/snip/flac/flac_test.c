...
wiced_result_t flac_app_audio_render_buffer_release(wiced_audio_render_buf_t *rend_buf, void* session_ptr)
{
    audio_buff_list_t*  buff_ptr;
    flac_player_t*      player = (flac_player_t *)session_ptr;
...
wiced_result_t flac_app_audio_render_buffer_push(wiced_flac_buffer_info_t *buff_info)
{
    ...
    if (wiced_audio_render_push(player->audio_renderer, &ar_buf) != WICED_SUCCESS)
    {
...
wiced_result_t flac_app_audio_render_init(flac_player_t* player, uint32_t sample_rate, uint16_t channels, uint16_t bits_per_sample, uint8_t frame_size)
{
    ...
    if (player->audio_renderer == NULL)
    {
        ...
        params.session_ptr  = player;
        params.cb_buf_release = flac_app_audio_render_buffer_release;
        params.cb_time_get = NULL;
        player->audio_renderer = wiced_audio_render_init( &params );
        if (player->audio_renderer == NULL)
        {
            FLAC_APP_PRINT(FLAC_LOG_DEBUG, ("Unable to create new FLAC Audio Renderer\r\n"));
            return WICED_ERROR;
        }
        ...
        if (wiced_audio_render_configure(player->audio_renderer, &audio_config)!= WICED_SUCCESS)
        {
...
wiced_result_t flac_app_audio_render_deinit(flac_player_t* player)
{
    ...
    if (player->audio_renderer != NULL)
    {
        wiced_audio_render_deinit(player->audio_renderer);
...
wiced_result_t flac_get_server_file_list(flac_player_t* player)
{
    ...
    if (result == WICED_SUCCESS )
    {
        ...
        FLAC_APP_PRINT(FLAC_LOG_ERROR, ("waiting for HTTP reply for %s -- %s\n", player->last_connected_host_name, FLAC_PLAYLIST_NAME ));
        ...
        do
        {
            ...
            {
                FLAC_APP_PRINT(FLAC_LOG_DEBUG, ("flac_get_server_file_list() wiced_tcp_receive() %d timeout!\r\n", result));
                result = WICED_SUCCESS; /* so we stay in our loop */
                wait_loop_count++;
                continue;
            }

            FLAC_APP_PRINT(FLAC_LOG_ERROR, ("flac_get_server_file_list() wiced_tcp_receive() result:%d reply_packet:%p\r\n", result, reply_packet));
...
_exit_http_thread:
    ...
    wiced_audio_render_stop(player->audio_renderer);
    wiced_audio_render_flush(player->audio_renderer);
...
wiced_result_t flac_network_up_default( flac_player_t* player, wiced_network_config_t config, const wiced_ip_setting_t* static_ip_settings )
{
    wiced_result_t result = WICED_SUCCESS;
    ...
    {
        FLAC_APP_PRINT(FLAC_LOG_INFO, ("Network already initialized\r\n"));
    ...
    FLAC_APP_PRINT(FLAC_LOG_INFO, ("Configuring wireless interface for RMC\r\n"));
    ...
    *data = (uint32_t)0;
    ...
    if (retval != WWD_SUCCESS && retval != WWD_UNSUPPORTED)
    {
        ...
        FLAC_APP_PRINT(FLAC_LOG_INFO, ("Could not turn off APSTA\r\n"));
    }
    ...
    *data = 1;
    ...
    if ( result != WICED_SUCCESS )
    {
        FLAC_APP_PRINT(FLAC_LOG_ERROR, ("Could not bring up network!\r\n"));
        return result;
    }
...
static flac_player_t *init_player(void)
{
    ...
    /* App */
    wiced_dct_read_lock( (void**)&player->dct_app, WICED_TRUE, DCT_APP_SECTION, 0, sizeof( flac_app_dct_t ) );
    ...
    result = flac_network_up_default(player, WICED_USE_EXTERNAL_DHCP_SERVER, NULL);
    if (result != WICED_SUCCESS)
    {
        /*
