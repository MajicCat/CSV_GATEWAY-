...
void platform_init_peripheral_irq_priorities( void )
{
    ...
    NVIC_SetPriority( Wake_Up_IRQn,     0 ); /* Apps core wake up */
    NVIC_SetPriority( WL2APPS_IRQn,     0 ); /* WLAN to apps core */
    ...
    NVIC_SetPriority( GPIOA_BANK0_IRQn, 7 ); /* GPIOA */
    NVIC_SetPriority( GPIOA_BANK1_IRQn, 7 ); /* GPIOA */
}
...
platform_result_t platform_reinit_wlan_stdio_uart( void )
{
    ...
    if ( STDIO_UART == WICED_UART_4 )
    {
        /* Re=-initialise UART standard I/O only if UART4 is used */
        return platform_stdio_init( &platform_uart_drivers[STDIO_UART], &platform_uart_peripherals[STDIO_UART], &stdio_config );
    }
    else
    {
